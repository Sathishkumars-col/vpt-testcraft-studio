import{c as Z,u as ee,r,j as e,T as se,U as $,F as q,X as E,a as ae,f as te,d as ne,o as ie,g as le,l as P}from"./index-Xl6yQnD7.js";import{T as U}from"./trash-2-YW-Nu2Wx.js";import{C as I}from"./chevron-down-DTfgq6GN.js";import{P as O}from"./plus-BpoVmSdz.js";import{L as ce}from"./layers-CND2HntF.js";const re=[["path",{d:"m18 15-6-6-6 6",key:"153udz"}]],L=Z("chevron-up",re),H=c=>{const d=c.split(".").pop().toLowerCase();return["pdf"].includes(d)?"PDF":["doc","docx"].includes(d)?"Word":["xls","xlsx"].includes(d)?"Excel":["csv"].includes(d)?"CSV":"File"};function he(){const c=ee(),d=r.useRef(null),C=r.useRef(null),[o,j]=r.useState([]),[v,z]=r.useState(!1),[n,g]=r.useState(null),[A,J]=r.useState({}),[m,N]=r.useState([]),[y,D]=r.useState(!1),[l,f]=r.useState(null),[w,M]=r.useState({}),R=()=>{try{return JSON.parse(localStorage.getItem("vpt-docs")||"[]").filter(s=>s.status==="parsed")}catch{return[]}},F=s=>new Promise(a=>{const i=new FileReader;i.onload=t=>a(t.target.result||""),i.onerror=()=>a(""),i.readAsText(s)}),_=s=>{const i=Array.from(s.target.files||[]).map(t=>({file:t,name:t.name,type:H(t.name),size:t.size}));j(t=>[...t,...i]),s.target.value=""},B=s=>j(a=>a.filter((i,t)=>t!==s)),W=async()=>{const s=R();if(o.length===0){c.info("Upload test scenarios/cases to analyze");return}if(s.length===0){c.info("No parsed documents in Document Hub to compare against");return}z(!0),g(null);try{const a=await Promise.all(o.map(async u=>{const S=await F(u.file);return`--- File: ${u.name} ---
