import{c as Z,u as ee,r,j as e,T as se,U as $,F as q,X as E,a as ae,f as te,d as ne,o as le,g as ie,l as P}from"./index-Du-vsTNH.js";import{T as U}from"./trash-2-Dstcv7Ia.js";import{C as I}from"./chevron-down-Cr5Ki6_p.js";import{P as O}from"./plus-BDKAx8cC.js";import{L as ce}from"./layers-Bcw8va8J.js";const re=[["path",{d:"m18 15-6-6-6 6",key:"153udz"}]],L=Z("chevron-up",re),H=c=>{const d=c.split(".").pop().toLowerCase();return["pdf"].includes(d)?"PDF":["doc","docx"].includes(d)?"Word":["xls","xlsx"].includes(d)?"Excel":["csv"].includes(d)?"CSV":"File"};function he(){const c=ee(),d=r.useRef(null),C=r.useRef(null),[o,j]=r.useState([]),[v,z]=r.useState(!1),[n,g]=r.useState(null),[A,J]=r.useState({}),[m,N]=r.useState([]),[y,D]=r.useState(!1),[i,f]=r.useState(null),[w,M]=r.useState({}),R=()=>{try{return JSON.parse(localStorage.getItem("vpt-docs")||"[]").filter(s=>s.status==="parsed")}catch{return[]}},F=s=>new Promise(a=>{const l=new FileReader;l.onload=t=>a(t.target.result||""),l.onerror=()=>a(""),l.readAsText(s)}),_=s=>{const l=Array.from(s.target.files||[]).map(t=>({file:t,name:t.name,type:H(t.name),size:t.size}));j(t=>[...t,...l]),s.target.value=""},B=s=>j(a=>a.filter((l,t)=>t!==s)),W=async()=>{const s=R();if(o.length===0){c.info("Upload test scenarios/cases to analyze");return}if(s.length===0){c.info("No parsed documents in Document Hub to compare against");return}z(!0),g(null);try{const a=await Promise.all(o.map(async u=>{const S=await F(u.file);return`--- File: ${u.name} ---
...
}`,p=await P(t,"test-gap-analysis"),h=p.reply||p.response||"",x=h.match(/\{[\s\S]*\}/);x?(g(JSON.parse(x[0])),c.success("Gap analysis complete")):(c.info("AI returned analysis but could not parse structured results"),g({summary:h,gaps:[],additionalTests:[],coverageScore:0,totalRequirements:0,coveredRequirements:0}))}catch(a){console.error("Gap analysis failed:",a),c.info("AI unavailable — check if the server is running")}finally{z(!1)}},Y=s=>{const l=Array.from(s.target.files||[]).map(t=>({file:t,name:t.name,type:H(t.name),size:t.size}));N(t=>[...t,...l]),s.target.value=""},Q=s=>N(a=>a.filter((l,t)=>t!==s)),V=async()=>{if(m.length===0){c.info("Upload test case files to check for duplicates");return}D(!0),f(null);try{const a=`You are a test case analysis expert. Analyze the following test cases/scenarios to find duplicates, near-duplicates, and overlapping test coverage.
